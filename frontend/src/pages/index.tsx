...
import {
  Github, Linkedin, Play, RotateCcw, ExternalLink
} from 'lucide-react'
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-surface-0 font-sans">
      ...
      {/* ── Header ── */}
      <header className="sticky top-0 z-50 bg-surface-0/95 backdrop-blur border-b border-border">
        <div className="max-w-screen-xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <a href="https://github.com/kenil0509"
               target="_blank"
            ...
            <a href="https://www.linkedin.com/in/kenil-sutariya-39b277213/"
               target="_blank"
               className="flex items-center gap-1.5 text-xs font-mono px-3 py-1.5 rounded-full border border-border hover:bg-surface-1 transition-colors text-gray-500">
      ...
      <section className="max-w-screen-xl mx-auto px-6 py-12 border-b border-border">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-gray-500 text-lg max-w-2xl leading-relaxed">
            A production-grade multi-agent pipeline that transcribes, analyzes, embeds, and
            extracts monetization signals from creator content — built specifically for CreatorJoy's stack.
          </p>
        </motion.div>
      </section>
      ...
      <section className="max-w-screen-xl mx-auto px-6 py-8 border-b border-border">
        ...
        <div className="flex gap-3 mb-3">
          <input
            ...
            className="flex-1 font-mono text-sm px-4 py-3 rounded-xl border border-border-strong bg-surface-1 focus:outline-none focus:border-brand-500 focus:bg-white transition-colors placeholder:text-gray-400"
            disabled={isRunning}
          />
          {!isRunning ? (
            <button
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-8 grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-8">
        ...
        <div className="space-y-4">
          ...
          {/* Stage 1: Ingestion */}
          <StageCard
            title="Stage 1 — Ingestion + Transcription"
            subtitle="yt-dlp · OpenAI Whisper · Speaker diarization"
            icon="📥"
            status={stages.ingestion.status}
            progress={stages.ingestion.progress}
            message={stages.ingestion.message}
          >
            <TranscriptViewer />
          </StageCard>
          ...
          {/* Stage 2: Agents */}
          <StageCard
            title="Stage 2 — LangGraph Multi-Agent Analysis"
            subtitle="StateGraph · 4 specialized agents · shared state"
            icon="🧠"
            status={stages.agents.status}
            progress={stages.agents.progress}
            message={stages.agents.message}
          >
            <AgentGraph />
          </StageCard>
          ...
          {/* Stage 3: Embedding */}
          <StageCard
            title="Stage 3 — Chunking + Vector Embedding"
            subtitle="text-embedding-3-small · pgvector · cosine similarity"
            icon="🔢"
            status={stages.embedding.status}
            progress={stages.embedding.progress}
            message={stages.embedding.message}
          >
            <VectorGrid />
          </StageCard>
          ...
          {/* Stage 5: Output */}
          <StageCard
            title="Stage 5 — Creator Intelligence Output"
            subtitle="Offer architecture · content repurposing · monetization map"
            icon="✨"
            status={stages.repurpose.status}
            progress={stages.repurpose.progress}
            message={stages.repurpose.message}
          >
            <div className="space-y-4">
              <OfferCard />
              <ContentMap />
